...
fn lab_row(r: &sqlx::mysql::MySqlRow) -> Value {
    json!({
        ...
        "results_json": r.get::<Option<String>, _>("results_json"),
...
pub async fn create_lab_report(
    ...
    let result = sqlx::query(
        "INSERT INTO lab_reports
         (patient_id, visit_id, report_name, lab_name, notes, status, ordered_at, received_at, results_json)
         VALUES (?,?,?,?,?,?,?,?,?)"
    )
    .bind(patient_id)
    .bind(data.visit_id)
...
pub async fn update_lab_report(
    ...
    if data.results_json.is_some() { sets.push("lr.results_json = ?") }
    ...
    if let Some(ref v) = data.results_json { q = q.bind(v) }
