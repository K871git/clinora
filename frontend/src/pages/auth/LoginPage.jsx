...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      else if (status === 403) setError('Your account is inactive. Contact the clinic administrator.')
      else if (status === 422) {
        const errors = err.response?.data?.errors
        setError(errors ? Object.values(errors).flat()[0] : 'Please check your input and try again.')
      } else if (!err.response) setError('Cannot reach the server. Make sure the backend is running.')
